...
# we are not exploring the functions of the top 5 significant genes in gout and SA vs healthy because of wordlimit reasons
# but I will keep this active because I need a couple dataframes I created here in later code chunks
...
# by human review for credibility reasons
...
#* *Comparison between gout VS SA and gout and SA vs healthy (and double checking with gout vs healthy)*
...
# we want to reorder the dataframe top10_in_gout and top10_in_sa so the rows appear in the same order as top10_genes_res (symbol)
top10_in_gout <- top10_in_gout[match(top10_genes_res$symbol, top10_in_gout$symbol), ]
top10_in_sa <- top10_in_sa[match(top10_genes_res$symbol, top10_in_sa$symbol), ]
...
# we will only use the gout one because in this case we want to check if the top 10 genes in the DESeq2 analysis are differentially
# expressed in gout or not, to further validate them as potential markers
...
# this is the dataframe we will use to compare the top 10 genes in the DESeq2 analysis with the top 10 significant genes in sa,
# because in the DESeq2 analysis what we have are SA as differentially expressed and we want to know if we got the
# same genes as the top 10 significant genes in the SA vs healthy
